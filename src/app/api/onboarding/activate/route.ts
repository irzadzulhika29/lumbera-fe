...
export async function POST(request: Request) {
  ...
  if (!payload.onboarding_draft_id || !payload.onboarding_token) {
    return NextResponse.json(
      { message: "Payload aktivasi koperasi tidak lengkap" },
      { status: 400 },
    );
  }

  const upstreamResponse = await fetch(
    `${getApiBaseUrl()}/onboarding/drafts/${payload.onboarding_draft_id}/activate`,
    {
      method: "POST",
      headers: {
