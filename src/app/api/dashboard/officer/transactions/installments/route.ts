const INSTALLMENTS_TRANSACTIONS_PATH = "/transactions/installments";
...
export async function POST(request: Request) {
  ...
  const upstreamResponse = await fetch(
    `${getApiBaseUrl()}${INSTALLMENTS_TRANSACTIONS_PATH}`,
    {
      method: "POST",
      headers: {
  ...
  if (!upstreamResponse.ok) {
    const message =
      typeof responseBody === "object" && responseBody !== null
        ...
        : String(responseBody || "Gagal menyimpan transaksi angsuran");
