const STORE_DASHBOARD_PATH = "/store/dashboard";
...
export async function POST(request: Request) {
  ...
  const upstreamResponse = await fetch(`${getApiBaseUrl()}${STORE_DASHBOARD_PATH}`, {
    method: "GET",
    headers: {
    ...
    const message =
      typeof responseBody === "object" && responseBody !== null
        ? (responseBody as { message?: string }).message ||
          "Gagal mengambil dashboard toko"
        : String(responseBody || "Gagal mengambil dashboard toko");
