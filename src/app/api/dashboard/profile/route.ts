...
export async function POST(request: Request) {
  ...
  const upstreamResponse = await fetch(`${getApiBaseUrl()}${PROFILE_PATH}`, {
    method: "GET",
    headers: {
  ...
  if (!upstreamResponse.ok) {
    const message =
      typeof responseBody === "object" && responseBody !== null
        ? (responseBody as { message?: string }).message || "Gagal mengambil profil"
        : String(responseBody || "Gagal mengambil profil");
