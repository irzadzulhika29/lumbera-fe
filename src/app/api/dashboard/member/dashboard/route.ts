const MEMBER_DASHBOARD_PATH = "/cooperative-members/dashboard";
...
export async function POST(request: Request) {
  ...
  const upstreamResponse = await fetch(`${getApiBaseUrl()}${MEMBER_DASHBOARD_PATH}`, {
    method: "GET",
    headers: {
    ...
    const message =
      typeof responseBody === "object" && responseBody !== null
        ? (responseBody as { message?: string }).message ||
          "Gagal mengambil dashboard anggota"
        : String(responseBody || "Gagal mengambil dashboard anggota");
