const MEMBER_LOAN_APPLICATIONS_PATH = "/cooperative-members/loan-applications";
...
export async function POST(request: Request) {
  ...
  const upstreamResponse = await fetch(
    `${getApiBaseUrl()}${MEMBER_LOAN_APPLICATIONS_PATH}`,
    {
      method: "POST",
      headers: {
  ...
  if (!upstreamResponse.ok) {
    const message =
      typeof responseBody === "object" && responseBody !== null
        ...
        : String(responseBody || "Gagal mengajukan pinjaman");
